package com.liferay.samples.fbo.oidc.user.resolver;
...
public class CustomOIDCUserInfoProcessor extends OIDCUserInfoProcessor {
	...
	public long processUserInfo(
		...
		if (userId > 0) {
			...
			// Currently, we don't update user details when the user is recognized
			// You may rewrite the logic so that it updates roles for example
